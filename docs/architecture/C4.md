...
- `Directory.Api`:
  ...
  - `KubeClient`: consulta Services `NodePort` no Kubernetes para mapear endpoints.
  - Endpoints:
    - `GET /directory/regions` retorna `{name, online, clusterTcp, localTcp, lastChecked}`.
    - `GET /directory/spawn` retorna `{regionName, x, y, clusterTcp, localTcp}`.
    - `POST /directory/resolve` retorna `{currentRegion, nextRegion, online, clusterTcp, localTcp}`.
  - Semântica de endpoints:
    - `clusterTcp`: endereço interno (`<service>:9090`) para comunicação dentro do cluster.
    - `localTcp`: endereço externo (`localhost:<tcpNodePort>`) quando o `Service` é `NodePort`. Pode ser `null` se não houver `NodePort` definido.
- `World.Server`:
...
- `world.cmd`:
 ...
 - RBAC (Directory):
   - `ServiceAccount`, `Role` e `RoleBinding` permitem `get/list/watch` de `services` no namespace para descoberta dinâmica via API do Kubernetes.
