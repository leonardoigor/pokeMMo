...
- `Gateway` (NGINX):
  - Rotas para `Auth`, `Users` e `Directory`.
  - Não acoplado aos mundos dinâmicos.
- `Auth` (`8080`): autenticação, JWT, Postgres.
- `Users` (`8081`): perfis e personagens, Postgres/Redis.
...
- `infra.cmd up`:
  - Build/push: `Auth`, `Users`, `Directory`.
  - `kubectl apply -k k8s` sem `World`.
  - `rollout restart`: `auth`, `users`, `gateway`, `directory`, `postgres`, `redis`.
- `world.cmd`:
...
- Centralização de logs:
  ...
  - Fluxo: Serviços (`Auth`, `Users`, `Gateway`, `Directory`, `World`) → `Elastic Agent` → `Elasticsearch` → visualização no `Kibana`.
- Kibana:
